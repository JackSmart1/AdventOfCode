interface SchWithNumbers {
  number: string,
  indexOfFirst?: number
}

const day3Task2 = (input) => {
  let total = 0;
  let index = 0;
  let splitSchematic = input.split("\n").map((s) => {
    index++;
    let first = true;
    let partNumbers: SchWithNumbers[] = [];
    let partNumber : SchWithNumbers= {
      number: "",
    };
    s.split("").forEach((d, i) => {
      if (d.match(/\d/)) {
  ...
  splitSchematic.forEach((element, i) => {
    ...
    let secondRowBelow = splitSchematic.find((s) => s.index === element.index + 2);

    if (element.partNumbers.length >= 1) {
      element.partNumbers.forEach((partNumber, index) => {
        let numbersAndOnechar = element.schmatic.slice(
          partNumber.indexOfFirst === 0 ? 0 : partNumber.indexOfFirst - 1,
          partNumber.indexOfFirst + partNumber.number.length + 1
        );
        numbersAndOnechar
        console.log(element.schmatic.charAt(partNumber.indexOfFirst + partNumber.number.length))
        console.log(element.schmatic.charAt(partNumber.indexOfFirst + partNumber.number.length + 1))
        if (
...
console.log(day3Task2(schematic2));
