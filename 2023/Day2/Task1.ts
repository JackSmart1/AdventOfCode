const main = () => {
  const testData = `Game 1: 13 red, 18 green; 5 green, 3 red, 5 blue; 5 green, 9 red, 6 blue; 3 blue, 3 green
...
Game 100: 12 blue, 2 green; 2 green, 4 red, 13 blue; 9 blue, 1 red, 2 green; 2 green, 3 red, 1 blue`;
  ...
  const getTotal = (games) => {
    ...
    games.forEach((g) => {
      ...
      let num = "";
      gameNumber.forEach((a: string) => (num += a));
      total += Number(num);
    });
  ...
  const isPossible = (game) => {
    let gameId = game.indexOf(":");
    console.log(game);
    const colourNumber = game.slice(gameId + 1).split(";");
    console.log(colourNumber);
    let isGamePossable = true;
    colourNumber.forEach((g) => {
      let splitGame = g.trim().split(",");
      ...
      splitGame.forEach((g) => {
        ...
        if (
          red > gameRules.red ||
          blue > gameRules.blue ||
          green > gameRules.green
        ) {
          isGamePossable = false;
        }
      });
    });
    return isGamePossable;
  };

  const task1 = (input: string) => {
    let gameArray = input.split("\n");

    let possableGamesArray = [];

    gameArray.forEach((g) => {
      if (isPossible(g)) {
        console.log(g);
        possableGamesArray.push(g);
      }
    });
    console.log(possableGamesArray);

    let possableGames = input.split("\n");
    let result = getTotal(possableGamesArray);
    console.log(result);
  };
